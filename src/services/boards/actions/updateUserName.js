...
export default function updateUserName(boardId, userId, userName) {
  Log.info('Service : Boards : updateUserName', { boardId, userId, userName });

  const board = this.boards[boardId];

  if (!board) {
    Log.warning('Service : Boards : updateUserName : tried to update the name of an user in a nonexistent board', { boardId });
    return;
  }
